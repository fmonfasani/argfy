# Mateo Llorente — zapateria.webshooks.com

| Dato | Valor |
|------|-------|
| Colaborador | Mateo Llorente |
| Tema | zapateria |
| Dominio | `zapateria.webshooks.com` |
| API | `api.zapateria.webshooks.com` |
| Containers | `zapateria-db`, `zapateria-api`, `zapateria-web` |
| Coolify Project | `ws-zapateria` |
| Deadline | 2026-05-22 |
...
3. Solo ves tu proyecto `ws-zapateria` — no podés ver otros proyectos ni config del servidor

## Tu dominio

`zapateria.webshooks.com` ya apunta al VPS. Cuando deployes, Traefik emite SSL automáticamente via Let's Encrypt.
...
O desde Coolify UI:
1. Coolify → `ws-zapateria` → servicio → "Deploy"
...
# Reemplazar placeholders (zapateria + zapateria.webshooks.com):
...
Setealas en Coolify UI → `ws-zapateria` → Environment Variables. NUNCA en el repo.

Mínimas requeridas:
- `POSTGRES_DB=zapateria`
- `POSTGRES_USER=zapateria`
