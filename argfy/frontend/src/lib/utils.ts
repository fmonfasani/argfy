...
export function formatCondColor(value: number | null | undefined, thresholds: [number, number]): { text: string; color: string } {
  if (value == null) return { text: "-", color: "text-slate-400" }
  const text = value.toFixed(1)
  if (value < thresholds[0]) return { text, color: "text-green-400" }
  if (value < thresholds[1]) return { text, color: "text-amber-400" }
